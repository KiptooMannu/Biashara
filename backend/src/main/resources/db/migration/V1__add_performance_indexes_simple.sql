-- Performance Optimization Indexes for BIASHARA ERP (Non-Concurrent Version)
-- This version works with direct Supabase connections for index creation
-- Run this in Supabase SQL Editor or via direct connection (not pooler)
...
CREATE INDEX IF NOT EXISTS idx_sale_tenant_date_deleted 
    ON sale (tenant_id, sale_date DESC, deleted);

CREATE INDEX IF NOT EXISTS idx_sale_tenant_deleted 
    ON sale (tenant_id, deleted);

CREATE INDEX IF NOT EXISTS idx_sale_customer_tenant 
    ON sale (customer_id, tenant_id, deleted);

-- Invoice table - heavily used for financial KPIs
CREATE INDEX IF NOT EXISTS idx_invoice_tenant_status_deleted 
...
CREATE INDEX IF NOT EXISTS idx_invoice_due_date_tenant 
    ON invoice (due_date, tenant_id, deleted);

-- Customer table - frequently queried for top customers and growth
CREATE INDEX IF NOT EXISTS idx_customer_tenant_deleted 
    ON customer (tenant_id, deleted);

CREATE INDEX IF NOT EXISTS idx_customer_tenant_spent_deleted 
    ON customer (tenant_id, total_spent DESC, deleted);

CREATE INDEX IF NOT EXISTS idx_customer_created_tenant 
    ON customer (created_on DESC, tenant_id, deleted);

-- Product table - inventory and sales analytics
CREATE INDEX IF NOT EXISTS idx_product_tenant_deleted 
    ON product (tenant_id, deleted);

CREATE INDEX IF NOT EXISTS idx_product_tenant_stock_deleted 
...
CREATE INDEX IF NOT EXISTS idx_user_tenant_email_deleted 
...
CREATE INDEX IF NOT EXISTS idx_role_tenant_hierarchy 
    ON role (tenant_id, hierarchy_level, deleted);

-- Project Task table - Kanban functionality
CREATE INDEX IF NOT EXISTS idx_project_task_tenant_status_position 
    ON project_task (tenant_id, status, board_position, deleted);

CREATE INDEX IF NOT EXISTS idx_project_task_tenant_project 
...
CREATE INDEX IF NOT EXISTS idx_active_customers_tenant 
    ON customer (tenant_id, total_spent DESC) 
    WHERE deleted = false;

CREATE INDEX IF NOT EXISTS idx_active_employees_tenant 
    ON employee (tenant_id, performance_score DESC) 
    WHERE deleted = false AND active = true;

CREATE INDEX IF NOT EXISTS idx_low_stock_products_tenant 
...
CREATE INDEX IF NOT EXISTS idx_sale_date_tenant 
    ON sale (DATE_TRUNC('day', sale_date), tenant_id) 
    WHERE deleted = false;

CREATE INDEX IF NOT EXISTS idx_customer_growth_tenant 
    ON customer (DATE_TRUNC('day', created_on), tenant_id) 
    WHERE deleted = false;

CREATE INDEX IF NOT EXISTS idx_inventory_movement_tenant 
